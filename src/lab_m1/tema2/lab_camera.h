...
namespace timplemented
{
    ...
    public:
        ...
        void RotateThirdPerson_OY(float angle)
        {
            ...
            TranslateForward(3.0f);
            TranslateUpword(1.0f);
        ...
        glm::vec3 GetTargetPosition(car c)
        {
            return c.position;
        }

        glm::vec3 GetPosition(car c)
        {
            return GetTargetPosition(c) + (abs(c.acceleration) / 5 + c.maxSpeed) / c.maxSpeed * (2.0f * c.up - 4.0f *  c.forward);
        }

        glm::vec3 GetPositionUp(car c)
        {
            return GetTargetPosition(c) +  (30.0f * c.up);
        }

        void SetTarget(car c)
        {
            Set(GetPosition(c), GetTargetPosition(c), c.up);
        }
