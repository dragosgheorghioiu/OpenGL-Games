...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(8, 0.5f, 0));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(45.0f), glm::vec3(0, 1, 0));
        RenderMesh(meshes["teapot"], shaders["VertexNormal"], modelMatrix);
    }
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_1)) {
        fovy += 10 * deltaTime;
        projectionMatrix = glm::perspective(fovy, window->props.aspectRatio, 0.01f, 200.0f);
    }
    ...
    if (window->KeyHold(GLFW_KEY_3) && isOrtho) {
        orthoTop += deltaTime;
        orthoBottom -= deltaTime;
        projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, 0.01f, 200.0f);
    }

    if (window->KeyHold(GLFW_KEY_4) && isOrtho) {
        orthoTop -= deltaTime;
        orthoBottom += deltaTime;
        projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, 0.01f, 200.0f);
    }

    if (window->KeyHold(GLFW_KEY_5) && isOrtho) {
        orthoLeft += deltaTime;
        orthoRight -= deltaTime;
        projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, 0.01f, 200.0f);
    }

    if (window->KeyHold(GLFW_KEY_6) && isOrtho) {
        orthoLeft -= deltaTime;
        orthoRight += deltaTime;
        projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, 0.01f, 200.0f);
    }
...
void Lab5::OnKeyPress(int key, int mods)
{
    ...
    if ( key == GLFW_KEY_P )
    {
        projectionMatrix = glm::perspective(RADIANS(45.0f), window->props.aspectRatio, 0.01f, 200.0f);
        isOrtho = true;
    }
